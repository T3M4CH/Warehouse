...
public class ProductServiceTests
{
    ...
    public async Task GetProductsAsync_ShouldReturnSuccess_WhenProductsExist()
    {
        ...
        {
            new Animal { Id = 1, Name = "Product1", Weight = 10, PassId = "xasa" },
            new Food { Id = 2, Name = "Product2", Weight = 20, ExpiredData = DateTime.UtcNow }
        };
    ...
    public async Task AddProduct_ShouldReturnSuccess_WhenValidDataIsProvided(string name, double weight, string passId, string size, string? expiryDate, EProductType category, bool expectedResult)
    {
        ...
        _mockProductRepository.Setup(repo => repo.AddProduct(It.IsAny<Product>())).Returns(Task.CompletedTask);
    ...
    {
        var productId = 1;
        var expectedProduct = new Clothes { Id = productId, Name = "Product1", Weight = 10, Size = "M" };
    ...
    public async Task AddProduct_ShouldReturnSuccess_WhenProductIsAdded()
    {
        ...
        var addedProduct = new Animal { Name = "New Product", Weight = 10, PassId = "12345", Category = EProductType.Animals };

        _mockProductRepository.Setup(r => r.AddProduct(It.IsAny<Product>())).Verifiable();
        ...
        Assert.True(result.IsSuccess);
        _mockProductRepository.Verify(r => r.AddProduct(It.IsAny<Product>()), Times.Once);
    ...
    public async Task AddTestProducts_ShouldReturnSuccess_WhenProductsAdded()
    {
        ...
        _mockProductRepository.Setup(r => r.AddProducts(It.IsAny<IEnumerable<Product>>())).Verifiable();
        _mockUnitOfWork.Setup(u => u.CommitAsync()).Returns(Task.CompletedTask);

        var result = await _productService.AddTestProducts();

        Assert.True(result.IsSuccess);
        _mockProductRepository.Verify(r => r.AddProducts(It.IsAny<IEnumerable<Product>>()), Times.Once);
        _mockUnitOfWork.Verify(u => u.CommitAsync(), Times.Once);
    }
